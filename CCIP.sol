...
contract CCIP is CCIPReceiver, OwnerIsCreator, ABC {
    ...
    function decodeBytes(bytes memory _messageData) public pure returns (string memory) {
        return abi.decode(_messageData, (string)); // abi-decoding of the sent text
    }
    ...
    function _ccipReceive(
        ...
        // First try to extract the data for the token to target (address) and the amounts
        (address userReceiver, address tokenToGet) = processDataString(s_lastReceivedText);

        // Second approve usdc tokens to the router for the swap
        IERC20(usdc).approve(uniswapRouter, s_lastReceivedTokenAmount);

        // Third execute a swap from USDC to WETH to the token
